...
class GridMaker:
    ...
    def create(self):
        ...
        self.add_starting_point("vertical", 0, 0)
        for i in range(10):
        ...
        while blocked is False:
            ...
            # print(self.banned_words, self.history.words, len(self.history._memory))
    ...
    def find_word(self, axis, starting_point):
        ...
        matching_words = list(word for word in matching_words if len(word) <= max_length)
                    ...
                    if letter == '■':
                        ...
                        if starting_point.row == 9:
                            ...
                            if not start_other_axis in words_by_start:
                                ...
                                vertical_max = 9 - starting_point.row + len(start_other_axis)
                                vertical_words = words_by_start[start_other_axis]
                                vertical_match = False
                                for vertical_word in vertical_words:
                                    if len(vertical_word) <= vertical_max:
                                        vertical_match = True
                                        break
                                if vertical_match is False:
                                    match = False
                                    break
                    ...
                    if letter == '■':
                        ...
                        if starting_point.col == 9:
                            ...
                            if not start_other_axis in words_by_start:
                                ...
                                vertical_max = 9 - starting_point.col + len(start_other_axis)
                                vertical_words = words_by_start[start_other_axis]
                                vertical_match = False
                                for vertical_word in vertical_words:
                                    if len(vertical_word) <= vertical_max:
    ...
    def write(self, word, axis, starting_point):
        ...
        self.history.add(word, starting_point, axis)  # save before write

        if axis == "horizontal":
            self.set_row(word=word, row=starting_point.row, from_col=starting_point.col)
            if starting_point.col + word_len < 10:
                black_square = GridSquare((starting_point.col + word_len, starting_point.row))
        else:
            self.set_col(word=word, col=starting_point.col, from_row=starting_point.row)
...
max_len = 0
with open("fat_dictionnaire.txt", 'r', encoding='utf8') as reader:
