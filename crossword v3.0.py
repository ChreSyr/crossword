...
class GridMaker:
    ...
    def create(self):
        ...
        while self.current_pos:
            ...
            if letter is False:
                ...
                self.set_letter(letter, *starting_point)
    ...
    def find_letter(self, coord):
        ...
        space = list(self.get_row(row=coord.row, from_col=0))
        ...
        for letter in shuffled_alphabet:
            ...
            if letter == '■':
                ...
                if coord.row > 0 and self.get_letter(col=coord.col, row=coord.row - 1) != '■':
                    if coord.col > 0 and self.get_letter(col=coord.col - 1, row=coord.row - 1) != '■':
                        pass
                    elif coord.col < 9 and self.get_letter(col=coord.col + 1, row=coord.row - 1) != '■':
                        pass
                    elif coord.row > 1 and self.get_letter(col=coord.col, row=coord.row - 2) != '■':
                        pass
                    else:
                ...
                if coord.row == 9:
                    if coord.col > 1 and self.get_letter(col=coord.col - 2, row=coord.row) != '■':
                        pass
                    elif self.get_letter(col=coord.col - 1, row=coord.row - 1) != '■':
                        pass
                    else:
                        self.invalid_letters.append('■')
                        continue
                ...
                if coord == (8, 9) and self.get_letter(col=9, row=8) == '■':
                    self.invalid_letters.append('■')
                    continue
            ...
            while True:
                ...
                case = self.get_letter(col=before_case.col, row=before_case.row)
                if case == '■':
                    break
                if case == ' ':
    ...
    if grid is not None:
        ...
        if save == "yes":
            with open(f"crosswords/crossword n°{int(time.time())}.txt", 'w', encoding='utf8') as writer:
                ...
                writer.write(f"\nFound after {trials} trials, {grid_maker.rebranches} rebranches and "
                             f"{format_time(time=time_used, formatter='%H hours, %M minutes and %S seconds')}")
